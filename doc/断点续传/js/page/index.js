		...
		if (res.success) {
			for (let file of res.data) {
				if (file.fileName.length > 10) {
					$("#fileList ul").append(
						`<li class="fl-li"><img src="${getFileLogo(file.suffix)}"><label>${file.fileName.substring(0,10)}…</label><span class="tooltiptext">${file.fileName}</span></li>`
					);
				} else {
					$("#fileList ul").append(
						`<li class="fl-li"><img src="${getFileLogo(file.suffix)}"><label>${file.fileName}</label></li>`
					);
				}
			}
		}
	...
	let uploader = $.uploaderInit({
		...
		fileQueued: (file) => {
			...
			if (file.name.length > 10) {
				$("#fileList ul").append(
					`<li class="fl-li"><img src="${getFileLogo(file.ext)}"><label>${file.name.substring(0,10)}…</label><span class="tooltiptext">${file.name}</span></li>`
				);
			} else {
				$("#fileList ul").append(
					`<li class="fl-li"><img src="${getFileLogo(file.ext)}"><label>${file.name}</label></li>`
				);
			}
			$("#upcData").append(
